...
Supports 1, 2, or 3 columns layout, 3-5 options (A-C, A-D, A-E),
...
class OMRPDFGenerator:
    ...
    def _draw_header(
        ...
        # Draw Institution Logo if available
    ...
    def _draw_omr_bubble_grid(
        ...
        # Spacing and radius adjustments for 1, 2, or 3 columns
        if cols == 3:
            bubble_radius = 5.5
            option_spacing = 20.0
            label_offset_x = 42.0
        ...
        for col_idx in range(cols):
            col_x_start = 65.0 + (col_idx * col_width) + 12.0

            # Column Header Letters
            c.setFont("Helvetica-Bold", 9 if cols == 3 else 10)
            c.setFillColor(NAVY_HEX)
            ...
            for q_in_col in range(q_per_col):
                ...
                if q_num % 2 == 0:
                    c.setFillColor(LIGHT_GRAY)
                    c.rect(col_x_start - 6, row_y - 4, col_width - 16, row_height, fill=1, stroke=0)

                c.setFont("Helvetica-Bold", 8 if cols == 3 else 9)
                c.setFillColor(DARK_GRAY)
                ...
                for opt_idx in range(num_options):
                    ...
                    c.drawCentredString(bx, by - 2.0, options_letters[opt_idx])
